...
ARCHIVE_NAME = 'PeekTrace'
BACKUP_DIRS = [
	'C:/Users/uderos/backup', 
	'C:/Users/uderos/OneDrive/backup',
	'G:/coagulation/TOP/AM/Testlines/UDR/backup',
	'D:/backup' ]

BACKUP_EXTENSION_LIST = [ 	\
	'h', 'hpp', 'c', 'cpp', 'ini', 'rc', 	\
	'filters', 'vcxproj', 'sln', 
]

IGNORE_DIR_LIST = [ 
	'/Debug/', '/Release/', '/.vs/', '/ipch/', '/_Bin/' 	\
	'.cov', '.vs' ]
...
def backup_archive(archive_abs_fname):
	...
	for d in BACKUP_DIRS:
		if os.path.isdir(d):
			destination_abs_name = "{}/{}".format(d, archive_basename)
			shutil.copy(archive_abs_fname, destination_abs_name)
			if os.path.isfile(destination_abs_name):
				print("Backup file: {}".format(destination_abs_name))
			else:
				print("## Unable to create backup file: {}".format(destination_abs_name))
		else:
			print("## Backup directory <{}> not available".format(d))
